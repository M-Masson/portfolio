...
function Nav() {
    ...
    const nav = [
        {title: "portfolio", image1: imageAbout1, image2: imageAbout2, text: "À propos"},
    ...
    return (
        <div id="navbar">
            {nav.map((item, index) => {
                const isActive =(location.pathname === `/portfolio/` && location.hash === '' && item.title === "portfolio") || location.hash === `#${item.title}`;
                return (
                    <Link
                        key={index}
